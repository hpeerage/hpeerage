...
// 8. Dynamic Content Loading with Fallback
const fallbackData = {
    earn: [
        { id: "playit", category: "Hospitality TECH", title: "Playit Platform", description: "All-in-one management for motels & PC areas.", image: "images/playit.png", url: "https://playit.kr", size: "large" },
        { id: "aria", category: "Branding", title: "ARIA Curation", description: "Premium aesthetic layout for arts & lifestyle.", image: "images/aria_ui.png", url: "#", size: "medium" }
    ],
    great: [
        { type: "slider", title: "The Master's Blueprint", description: "From initial sketch to pixel-perfect delivery.", after_img: "images/mastery_ui.png", before_img: "images/mastery_blueprint.png" }
    ],
    system: { links: { email: "contact@hpeerage.com", github: "#", linkedin: "#" } }
};

async function loadDynamicContent() {
    // Check for local file protocol to avoid CORS errors in console
    if (window.location.protocol === 'file:') {
        console.groupCollapsed('Local Development Mode: File protocol detected');
        console.info('Bypassing fetch for data/projects.json to avoid CORS browser warnings.');
        console.info('Using fallback data for local offline preview.');
        console.groupEnd();
...
function renderGreatSection(mastery) {
    ...
    container.innerHTML = mastery.map(m => {
        if(m.type === 'slider') {
            return `<div class="great-card master-blueprint large"><div class="card-glass-effect"></div><div class="comparison-slider"><img src="${m.after_img}" class="comparison-after"><img src="${m.before_img}" class="comparison-before"></div><div class="card-info"><h3>${m.title}</h3><p>${m.description}</p></div></div>`;
        } else {
